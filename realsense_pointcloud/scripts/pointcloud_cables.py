...
class cable_preprocesser(object):
    ...
    # Solve all of perception here...
        ...
        ret,plane_thresh_im = cv2.threshold(plane_points_generators,127,255,cv2.THRESH_BINARY)
        plane_thresh_im=np.copy(plane_points_generators)
        plane_thresh_im[plane_thresh_im!=0]=255
        # cv2.imshow('plane_thresh_im',plane_thresh_im)
        # cv2.waitKey(0)
        ######################## PUBLISHING
        ...
        self.imgmsg_depth_cables = self.bridge.cv2_to_imgmsg(plane_points_generators, "16UC1")
        ...
        checkpoint1 = str((time.time() - start_time))
        rospy.loginfo("time:")
        rospy.loginfo(checkpoint1)
