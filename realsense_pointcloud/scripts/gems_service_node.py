...
class gem_detector(object):
    def __init__(self):
        ...
        self.pruning_point_posestamped=PoseStamped()
        # Publishers
    ...
    def start(self,req):
        ...
        self.finger_pose_msg=req.finger
        rospy.loginfo("finger_pose acquired!")
    ...
    def publish_finger_point(self):
        self.pruning_point_posestamped=self.finger_pose_msg
    ...
    def reading_callback(self,color_image_rect, depth_image_rect):
        ...
        ################################################# READING
            ...
            remove_idx= np.random.randint(0,fake_image_vector.shape[0],int(fake_image_vector.shape[0]*0.6)) # removing 0.n% of data
            ...
            self.pruning_point_posestamped=point
            self.check=True
            rospy.loginfo("Pruning Point Found!")


def gems_req(req): # here we call the handler
    # grapes_wrt_tag_t = PoseArray()
    rospy.logwarn("Service request recieved!")
    try:
        class_gem = gem_detector()
        class_gem.start(req)
        # print(class_gem.check)
        while class_gem.check==False:
            rospy.sleep(0.01)

        class_gem.check=False

        return gems_serviceResponse(pruning_point=class_gem.pruning_point_posestamped)
    except rospy.ServiceException as e:
        rospy.logerr("Service call failed: %s"%e)


def gems_service_server():
    rospy.init_node('gems_service_node', anonymous=True) # initialize the service node
    rospy.loginfo("gems service node ready")
    s = rospy.Service('gems_service_srv',  gems_service, gems_req)
    rospy.spin()
